...
class User {
    ...
    public function updateProfile($id, $bio, $imageName) {
        try {
            ...
            error_log("User model - Updating profile for user $id");
            error_log("Bio: " . $bio);
            error_log("Image: " . $imageName);
            
            // Update both bio and profile image
            $query = "UPDATE users SET bio = ?, profile_image = ? WHERE id = ?";
            $stmt = $db->prepare($query);
            
            // Execute the query and check for errors
            $result = $stmt->execute([$bio, $imageName, $id]);
            
            if (!$result) {
                $error = $stmt->errorInfo();
                error_log("Database update failed: " . print_r($error, true));
                return false;
            }
            ...
            error_log("Profile update successful in database");
            return true;
        } catch (\Exception $e) {
            error_log("Profile update error in User model: " . $e->getMessage());
            return false;
        }
    }
